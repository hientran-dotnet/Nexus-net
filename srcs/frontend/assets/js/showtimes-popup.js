...
function bookTicket(showtimeId, time, price) {
  ...
  const selectedDate = document.querySelector('.date-item.active .date-number').textContent;
  const selectedDay = document.querySelector('.date-item.active .date-day').textContent;
  
  // Có thể chuyển sang trang booking hoặc hiện popup xác nhận
  if (confirm(`Đặt vé xem "${movieTitle}"\nNgày: ${selectedDay} ${selectedDate}\nGiờ: ${time}\nGiá: ${price.toLocaleString('vi-VN')}đ`)) {
    window.location.href = `/booking.html?showtime_id=${showtimeId}&movie=${encodeURIComponent(movieTitle)}&date=${document.querySelector('.date-item.active').dataset.date}&time=${time}&price=${price}`;
  }
}